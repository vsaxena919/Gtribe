...
class LinkedIn extends OAuth2
{
    ...
    public function getUserProfile()
    {
        ...
        // LinkedIn Workarround.

        // First Name.
        $firstName = $data->filter( 'firstName' );
        $firstNameLocale = $firstName->filter( 'preferredLocale' );
        $userProfile->firstName = $data->filter( 'firstName' )->filter( 'localized' )->get( $firstNameLocale->get( 'language' ) . '_' . $firstNameLocale->get( 'country' ) );

        // Last Name.
        $lastName = $data->filter( 'lastName' );
        $lastNameLocale = $lastName->filter( 'preferredLocale' );
        $userProfile->lastName = $data->filter( 'lastName' )->filter( 'localized' )->get( $lastNameLocale->get( 'language' ).'_'.$lastNameLocale->get('country'));
