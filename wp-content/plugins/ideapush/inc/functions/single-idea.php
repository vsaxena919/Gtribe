...
function idea_push_add_content_after_title( $content ) {
    if ( is_single() && 'idea' == get_post_type()) {
        ...
        if(in_array($showBoardTo, $currentUserRole) || $showBoardTo == "Everyone"){
            ...
            //login form
            $custom_content .= '<div style="display: none;" id="create-user-form" data-first="'.__( 'First name', 'ideapush' ).'" data-last="'.__( 'Last name', 'ideapush' ).'" data-email="'.__( 'Email', 'ideapush' ).'" data-email-confirm="'.__( 'Confirm email', 'ideapush' ).'"></div>';
