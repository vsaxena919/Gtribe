<?php
    ...
    function idea_push_list_items($boardId,$order,$status,$tag){
                        ...
                        $html .= '<div class="idea-item-content-container">';
                            ...
                            if($ideapush_is_pro == "YES" && isset($options['idea_push_show_image_inline'])){
                                if(get_the_post_thumbnail_url($ideaPost->ID) !== false){
                                    $html .= '<img class="idea-item-attachment-inline" src="'.get_the_post_thumbnail_url($ideaPost->ID).'">';
                                }    
                            }
    ...
    function idea_push_fetch_new_list_items(){
        ...
        echo idea_push_list_items($boardNumber,$sortFilter,$statusFilter,$tagFilter);
    ...
    function idea_push_get_tags_and_attachments($postId){
            ...
            if($ideapush_is_pro == "YES" && isset($options['idea_push_show_custom_fields'])){
                ...
                foreach($postMeta as $key => $value){
                    ...
                    if(strpos($key, 'ideapush-custom-field-') !== false) {
                        ...
                        //dont show if youtube or vimeo link as we want to show that inline instead
                        if(strpos(strtolower($value[0]), 'youtube.com') === false && strpos(strtolower($value[0]), 'vimeo.com') === false){
                            $customFieldBuilder .= '<strong>'.str_replace("ideapush-custom-field-","",$key).':</strong> ';

                            //we are going to check if date or website
                            if(substr( $value[0], 0, 4 ) === "http"){
                                $customFieldBuilder .= '<a href="'.$value[0].'" target="_blank">'.$value[0].'</a>, ';
                            } elseif(preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/",$value[0])){
                                $customFieldBuilder .= date_i18n(get_option( 'date_format' ),strtotime($value[0])).', ';   
                            } else {
                                $customFieldBuilder .= $value[0].', ';
                            }
    ...
    function idea_push_get_idea_meta($ideaPost,$status,$boardSettings){
        ...
        //date published
        $html .= '<span class="idea-item-date">'.esc_html(human_time_diff(strtotime($ideaPost->post_date), current_time( 'timestamp' ) )).' '.__( 'ago','ideapush' ).'</span>';
