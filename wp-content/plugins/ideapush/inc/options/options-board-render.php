    ...
    function idea_push_render_board($data,$boardId = '',$boardName = ''){
        ...
        $board_settings = array(
            ...
            array(
                ...
                'type' => 'select',
                'tooltip' => __('With this feature enabled users will be allocated a certain number of votes (dictated in the below setting). Once the user hits this number of votes they can no longer vote anymore until the ideas they voted on reach the threshold score, they retract their vote or the idea gets deleted. For example if a user votes for idea X and idea X reaches the vote threshold score they will be refunded 1 vote in their bank. Note, admins can manually change the votes of a user in the users profile edit page.','ideapush'),
                'options' => array(
                    'No' => 'No',
                    'Yes' => 'Yes',
                ),
            ...
            $html .= '<table>';
                $html .= '<tbody>';
                    ...
                    foreach($board_settings as $board_setting){
                        ...
                        $html .= '<tr '.$style.' class="'.$board_setting['name'].' '.$board_id.'-'.$board_setting['name'].'">';   
                            //label
                            $html .= '<td>';
