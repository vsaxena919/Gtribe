...
add_action( 'init', 'yz_instagram_widget_process_authentication' );

function yz_instagram_widget_process_authentication() {

    if ( ! isset( $_GET['action'] ) || 'yz_account_connect' != $_GET['action'] || 'Instagram' != $_GET['provider'] || ! is_user_logged_in()  ) {
        return false;
    }
    ...
    try {
        ...
        // Config Data.
        $config = array( 
            'callback'   => home_url( '/?action=yz_account_connect&provider=Instagram' ),
