<?php
if (!class_exists('CFContributioneextension')) {

    class CFContributioneextension {

        public static function init() {
            //contribution extension table ajax and settings hooks
            add_action('wp_ajax_nopriv_updateextension', array(__CLASS__, 'ajax_extension'));
            add_action('wp_ajax_updateextension', array(__CLASS__, 'ajax_extension'));
            add_action('wp_ajax_nopriv_rejectextension', array(__CLASS__, 'rejectextension'));
            add_action('wp_ajax_rejectextension', array(__CLASS__, 'rejectextension'));
            add_filter('woocommerce_cf_settings_tabs_array', array(__CLASS__, 'crowdfunding_Contributioneextension_tab'), 1500);
        }
                        ...
                        foreach ($comp_modification_new as $innerarrays_new) {
                            if (is_array($innerarrays_new)) {
                                ...
                                if ($existendmethod == 'Target Goal') {
                                    $contributed_amount = $order_total = fp_wpml_multi_currency(get_post_meta($product_id, '_crowdfundingtotalprice', true));
                                    ...
                                    $local_current_time = strtotime(date('d-m-Y H:i:s'));
