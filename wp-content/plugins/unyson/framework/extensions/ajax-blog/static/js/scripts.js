var cruminaAjaxBlog = {
    ...
    reinitScriptsIfNeeded: function () {
        ...
        $gridContainer.imagesLoaded(function () {
            ...
            jQuery.fn.matchHeight._apply($gridContainer.find('[data-mh="news-grid"] > *'));

            CRUMINA.mediaPopups();

            //Match height init
            if (typeof $.fn.matchHeight !== 'undefined') {
                _this.$grid.find('.ajax-col-equal-height').matchHeight({property: 'min-height'});
                _this.$grid.find('.ajax-col-content-equal-height').matchHeight({property: 'min-height'});
            }
        });
    },
    getPostsProcessing: {
        ...
        beforeSendReplace: function () {
            ...
            jQuery('html, body').animate({
                scrollTop: jQuery(_this.$filter).offset().top
            }, 400);
