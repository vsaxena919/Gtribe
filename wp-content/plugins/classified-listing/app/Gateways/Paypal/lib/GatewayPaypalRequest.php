...
class GatewayPaypalRequest {
	...
	public function __construct( $gateway ) {
		...
		$this->notify_url = Link::payment_api_request_url($gateway);
	}
	...
	protected function get_paypal_args( $Payment ) {
		...
		return apply_filters( 'rtcl_paypal_args', array_merge(
			array(
				...
				'rm'            => is_ssl() ? 2 : 1,
				'upload'        => 1,
				'return'        => esc_url_raw( add_query_arg( 'utm_nooverride', '1', $this->gateway->get_return_url( $Payment ) ) ),
//				'cancel_return' => esc_url_raw( $Payment->get_cancel_order_url_raw() ),
...
//				'invoice'       => $this->limit_length( $this->gateway->get_option( 'invoice_prefix' ) . $Payment->get_order_number(), 127 ),
    ...
    protected function get_item_args( $payment ) {
        $item                = array();
        $item['item_name_1'] = $this->limit_length( $payment->get_listing_title(), 127 );
