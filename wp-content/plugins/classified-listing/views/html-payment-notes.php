...
<ul class="rtcl_payment_notes">
	...
	if ( $notes ) {
		foreach ( $notes as $note ) {
			...
			$css_class   = apply_filters( 'rtcl_payment_note_class', array_filter( $css_class ), $note );
			?>
            <li rel="<?php echo absint( $note->id ); ?>" class="<?php echo esc_attr( implode( ' ', $css_class ) ); ?>">
                ...
                <p class="meta">
                    <abbr class="exact-date"
						...
						echo esc_html( sprintf( __( '%1$s at %2$s', 'classified-listing' ), $note->date_created->date_i18n( wc_date_format() ), $note->date_created->date_i18n( wc_time_format() ) ) );
						?>
                    </abbr>
					<?php
